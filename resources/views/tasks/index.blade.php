                                            ...
                                            <div class="d-flex overflow-scroll">
                                                <div class="col-md-3 ">
                                                    <div class="card">
                                                        ...
                                                        <div class="card-body empty-list" data-status="pending" style="height:30vh">
                                                            <ul class="list-group connectedSortable shadow-lg" id="pending-item-drop">
                                                                ...
                                                                <li class="list-group-item" project="{{$task->taskProject->id}}" task-id="{{ $task->id }}" status-changer="{{$task->taskProject->changer}}" onclick="onTaskNameClick('{{ $task->id }}')">{{ $task->task_name }}
                                                    ...
                                                    <div class="card">
                                                        ...
                                                        <div class="card-body empty-list" data-status="on-progress">
                                                            <ul class="list-group connectedSortable shadow-lg" id="on-progress-item-drop">
                                                                ...
                                                                <li class="list-group-item" project="{{$task->taskProject->id}}" status-changer="{{$task->taskProject->changer}}" task-id="{{ $task->id }}" onclick="onTaskNameClick('{{ $task->id }}')">{{ $task->task_name }}
            ...
            $("#pending-item-drop, #on-progress-item-drop, #completed-item-drop, #accepted-item-drop,.empty-list").sortable({
                ...
                update: function(event, ui) {
                    if (isUserInteraction) {
                        ...
                        var changer = ui.item.attr("status-changer");
                        var project = ui.item.attr("project");


                        var currentUserId = "{{Auth::user()->id}}";
                        var SuperAdmin = "{{Auth::user()->is_super === 1 }}";
                        ...
                        console.log("status changer: " + changer);
                        console.log("cuurenUserID: " + currentUserId);
                        console.log("project: " + project);
                        if (SuperAdmin || changer.includes(currentUserId)) {
                            // Your code to update the task status
                            $.ajax({
                                url: "/tasks/" + taskId,
                                method: "PATCH",
                                headers: {
                            ...
                            console.log("CurrentUserId does not match any value in the changer array.");
                        }
                    }
